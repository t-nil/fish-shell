...
fn var_to_int_or(var: Option<EnvVar>, default: isize) -> isize {
    match var {
        Some(s) => {
            let proposed = fish_wcstoi(&s.as_string());
...
/// Called to initialize the termsize.
/// The pointer is to an dyn Environment, but has the wrong type to satisfy cxx.
...
add_test!("test_termsize", || {
    ...
    *STUBBY_TERMSIZE.lock().unwrap() = Some(Termsize {
    ...
    parser
        .vars()
        .set(L!("COLUMNS"), EnvMode::GLOBAL, vec![L!("75").to_owned()]);
    parser
        .vars()
        .set(L!("LINES"), EnvMode::GLOBAL, vec![L!("150").to_owned()]);
    ts.handle_columns_lines_var_change(parser.vars());
    assert_eq!(ts.last(), Termsize::new(75, 150));
    assert_eq!(parser.vars().get(L!("COLUMNS")).unwrap().as_string(), "75");
    assert_eq!(parser.vars().get(L!("LINES")).unwrap().as_string(), "150");

    parser
        .vars()
        .set(L!("COLUMNS"), EnvMode::GLOBAL, vec![L!("33").to_owned()]);
    ts.handle_columns_lines_var_change(parser.vars());
    ...
    parser
        .vars()
        .set(L!("COLUMNS"), EnvMode::GLOBAL, vec![L!("83").to_owned()]);
    parser
        .vars()
        .set(L!("LINES"), EnvMode::GLOBAL, vec![L!("38").to_owned()]);
