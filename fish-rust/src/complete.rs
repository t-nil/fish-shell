...
use crate::{
    ...
    path::{path_get_path, path_try_get_path, GetPathError},
...
impl Completion {
    ...
    pub fn with_desc(completion: WString, description: WString) -> Self {
        Self {
            completion,
            description,
            match_: StringFuzzyMatch::exact_match(),
...
impl CompletionReceiver {
    ...
    /// Adds a completion. Returns `true` on success, `false` if this would overflow the limit.
    #[must_use]
...
static s_completion_map: Mutex<CompletionEntryMap> = Mutex::new(BTreeMap::new());
...
pub fn sort_and_prioritize(comps: &mut Vec<Completion>, flags: CompletionRequestOptions) {
    ...
    if flags.autosuggestion {
        comps.sort_by(|a, b| match (a.match_.case_fold, b.match_.case_fold) {
...
impl<'ctx> Completer<'ctx> {
    pub fn new(ctx: &'ctx OperationContext, flags: CompletionRequestOptions) -> Self {
        Self {
            ctx,
            flags,
    ...
    pub fn perform_for_commandline(&mut self, mut cmdline: WString) {
        ...
        let decrement = if let Some(parser) = self.ctx.parser.as_ref() {
            ...
            *level += 1;
            ...
            Some(Cleanup::new(parser, |parser| {
                let level = &mut parser.libdata_mut().pods.complete_recursion_level;
                *level -= 1;
            }))
        } else {
    ...
    fn complete_cmd(&mut self, str_cmd: WString) {
        ...
        if str_cmd.is_empty() || (!str_cmd.contains('/') && str_cmd.as_char_slice()[0] != '~') {
            ...
            let possible_comp: Vec<_> = function_get_names(include_hidden)
                .into_iter()
    ...
    fn complete_param_for_command(
        ...
        let all_options: Vec<Vec<CompleteEntryOpt>> = s_completion_map
            .lock()
            .unwrap()
            .iter()
            .filter_map(|(idx, completion)| {
                let match_ = if idx.is_path { &path } else { &cmd };
                if wildcard_match(match_, &idx.name, false) {
        ...
        for options in all_options {
                        ...
                        for o in &options {
                            ...
                            if o.typ == CompleteOptionType::Short {
                                if let Some(prev_short_opt_pos) = prev_short_opt_pos {
                                    match_ = prev_short_opt_pos + 1 == popt.len()
                                        && o.option.char_at(0) == popt.char_at(prev_short_opt_pos);
                                }
                            } else if o.typ == CompleteOptionType::DoubleLong {
                                match_ = param_match(o, popt);
                            }
                            if match_ && self.conditions_test(&o.conditions) {
                                if o.result_mode.requires_param {
                                    use_common = false;
                                }
                                if o.result_mode.no_files {
            ...
            for o in &options {
                ...
                let match_ = string_prefixes_string(s, &whole_opt);
                if !match_ {
                ...
                if o.typ == CompleteOptionType::DoubleLong && (has_arg && !req_arg) {
                    ...
                    if !self.completions.add(Completion {
                        completion,
                        description: o.localized_desc().to_owned(),
                        flags: flags | CompleteFlags::NO_SPACE,
                        ..Default::default()
                    }) {
                        return false;
                    }
                ...
                if !self.completions.add(Completion {
                    completion: whole_opt.slice_from(offset).to_owned(),
                    description: o.localized_desc().to_owned(),
                    flags,
                    ..Default::default()
                }) {
    ...
    fn complete_variable(&mut self, s: &wstr, start_offset: usize) -> bool {
        ...
        for env_name in self.ctx.vars.get_names(EnvMode::empty()) {
            ...
            let (comp, flags) = if !match_.requires_full_replacement() {
            ...
            let _ = self.completions.add(Completion {
                completion: comp,
                description: desc,
                match_,
                flags,
    ...
    fn try_complete_user(&mut self, s: &wstr) -> bool {
        ...
        while let Some(pw_name) = getpwent_name() {
            ...
            if string_prefixes_string(user_name, &pw_name) {
                ...
                let _ = self.completions.add(Completion {
                    completion: pw_name.slice_from(name_len).to_owned(),
                    description: desc,
                    flags: CompleteFlags::NO_SPACE,
                    ..Default::default()
                });
    ...
    fn walk_wrap_chain(
        ...
        for wt in targets {
            ...
            let Some((wrapped_command, wrapped_command_offset_in_wt)) =
                Option::zip(wrapped_command, wrapped_command_offset_in_wt) else {
                continue;
            };
    ...
    fn escape_opening_brackets(&mut self, argument: &wstr) {
        ...
        // unescaped version.
        let Some(unescaped_argument) =
            unescape_string(argument, UnescapeStringStyle::Script(UnescapeFlags::INCOMPLETE)) else {
            return;
        };
    ...
    fn mark_completions_duplicating_arguments(
        ...
        for comp in self.completions.get_list_mut() {
            let comp_str = &mut comp.completion;
            if !comp.flags.contains(CompleteFlags::REPLACES_TOKEN) {
...
fn parse_cmd_string(s: &wstr, vars: &dyn Environment) -> CmdString {
    let path = match path_try_get_path(s, vars) {
        // Resolve commands that use relative paths because we compare full paths with "complete -p".
        Ok(path) if !path.is_empty() && path.as_char_slice().first() != Some(&'/') => {
        ...
        Ok(path) => path,
        Err(GetPathError { err: _, best_path }) => best_path.unwrap_or_else(WString::new),
    };
...
fn complete_function_desc(f: &wstr) -> WString {
    if let Some(props) = function_get_props(f) {
        props.read().unwrap().description.clone()
    } else {
        WString::new()
    }
}
...
fn param_match2<'s>(e: &CompleteEntryOpt, optstr: &'s wstr) -> Option<&'s wstr> {
    ...
    if !e.option.starts_with(optstr.slice_from(cursor)) {
        return None;
    }
    cursor += e.option.len();
...
fn short_option_pos(arg: &wstr, options: &[CompleteEntryOpt]) -> Option<usize> {
    ...
    for (pos, arg_char) in arg.chars().enumerate().skip(1) {
        ...
        if let Some(match_) = match_ {
            if match_.result_mode.requires_param {
                return Some(pos);
            }
        } else {
...
pub fn append_completion(
    ...
    completions.push(Completion::new(comp, desc, match_, flags))
}
...
pub fn complete_add(
    ...
    let mut completion_map = s_completion_map.lock().expect("mutex poisoned");
...
pub fn complete_print(cmd: &wstr) -> WString {
    ...
    let completions = s_completion_map.lock().expect("poisoned mutex");
